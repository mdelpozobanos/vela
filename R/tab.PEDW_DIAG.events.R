...
#' @export
#'
tab.PEDW_DIAG.events <- function(...){

  # Define table specific variables
  calling_table = "PEDW_DIAG";
  table_vars = list(
    ...
    NOTES = "'[DIAG_NUM: ' || lpad(coalesce(@v{tab}.DIAG_NUM, 99), 2, 0) || ']'
          || '[ADMIS: ' || case
  ...
  # SPECIALY comes from SAILW0843V.PEDW_SPEC_CD with a left join. Add this at the end
  # args$sql_info$from = rbind(
  #   args$sql_info$from,
  #   data.frame(
