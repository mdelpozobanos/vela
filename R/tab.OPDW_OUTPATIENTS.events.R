...
#' @export
#'
tab.OPDW_OUTPATIENTS.events <- function(...){

  # Define table specific variables
  calling_table = "OPDW_OUTPATIENTS";
  table_vars = list(
    TAB = paste0("@T{", calling_table, "}"),
    tab = paste0("@t{", calling_table, "}"),
    src = calling_table,
    DT = "@v{tab}.ATTEND_DT",
    START_DT = "@v{tab}.ATTEND_DT",
    END_DT = "@v{tab}.ATTEND_DT",
    NOTES = "'' ||
        case
          when @t{tab}.ATTEND_CD is NULL then '[ATTEND_CD: invalid]'
          when @t{tab}.ATTEND_CD in ('5', '6') then '[ATTEND_CD: seen]'
          when @t{tab}.ATTEND_CD in ('2', '3', '4', '7', '8') then '[ATTEND_CD: not seen]'
          end",
    CODE = NA
  )
